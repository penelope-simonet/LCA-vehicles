function plot_figureS10_norm_carrying_capacities_EF_CTR_individual(obj)

% Computes the Norway-allocated planetary boundary
% share (Capacity to Reduce / CTR principle) for all 16 EF v3.1 categories
% used in this study, ranks them descending by their maximum exceedance

% Use : OS.plot_figureS10_norm_carrying_capacities_EF_CTR_individual()
...
for k = 1:n_cats_total
    cat_idx   = all_ids(k);
    PB_norway = NormFactors.PB_CTR(cat_idx);
...
output_pdf = fullfile(output_dir, 'figureS10_norm_carrying_capacities_EF_CTR_individual.pdf');
...
output_xlsx = fullfile(source_dir, 'figureS10_norm_carrying_capacities_EF_CTR_individual.xlsx');
...
pb_header = {'Rank', 'Category', 'Unit', 'PB_Norway_CTR', 'Max_share'};
...
for k = 1:n_cats
    ...
    box(ax, 'off');
    set(ax, 'FontSize', 12);
    ylabel(ax, 'Share of PB (Norway, CTR)', 'FontSize', 12);
...
legend(ax_leg, [h_ops h_emb h_inf h_tot h_pb], ...
    {'Operational', 'Embodied', 'Infrastructure', 'Total', 'Carrying capacity (Norway, CTR)'}, ...

function plot_figureS8_norm_carrying_capacities_EF_EV_individual(obj)

% Computes the Norway-allocated planetary boundary
% share (Economic Value / EV principle) for all 16 EF v3.1 categories used
% in this study, ranks them descending by their maximum exceedance

% Use : OS.plot_figureS8_norm_carrying_capacities_EF_EV_individual()
...
for k = 1:n_cats_total
    cat_idx   = all_ids(k);
    PB_norway = NormFactors.PB_EV(cat_idx);
...
output_pdf = fullfile(output_dir, 'figureS8_norm_carrying_capacities_EF_EV_individual.pdf');
...
output_xlsx = fullfile(source_dir, 'figureS8_norm_carrying_capacities_EF_EV_individual.xlsx');
...
pb_header = {'Rank', 'Category', 'Unit', 'PB_Norway_EV', 'Max_share'};
...
for k = 1:n_cats
    ...
    box(ax, 'off');
    set(ax, 'FontSize', 12);
    ylabel(ax, 'Share of PB (Norway, EV)', 'FontSize', 12);
...
legend(ax_leg, [h_ops h_emb h_inf h_tot h_pb], ...
    {'Operational', 'Embodied', 'Infrastructure', 'Total', 'Carrying capacity (Norway, EV)'}, ...

function plot_figureS5_norm_carrying_capacities_EF_EPCsect(obj)

% Same as plot_normalization_EF_planetary_boundaries.m, but Norway's
% planetary boundary allocation uses a sectoral Equal Per Capita (EPC)
% principle: Norway's per-capita share is further allocated to the
% "Other land transport" economic sector, based on that sector's share
% of Norwegian total gross value added.
% Source for PBs: Sala et al. (2020), Table 3.
% Source for sectoral EPC allocation: Hartmann et al. (2026), Scientific
% Data, 13, 970. Dataset: Zenodo, doi.org/10.5281/zenodo.17457702.

% Use : OS.plot_figureS5_norm_carrying_capacities_EF_EPCsect()

NormFactors = get_normalization_factors_EF_EPCsect();
PB_norway = NormFactors.PB_EPCsect;
...
ylabel(ax, 'Share of Norwegian carrying capacities (EPC by sector, log scale)', 'FontSize', 10);
grid(ax, 'on');
box(ax, 'off');
set(ax, 'FontSize', 10);
...
output_pdf = fullfile(output_dir, 'figureS5_norm_carrying_capacities_EF_EPCsect.pdf');
...
output_xlsx = fullfile(source_dir, 'figureS5_norm_carrying_capacities_EF_EPCsect.xlsx');
...
% Sheet 2 : carrying capacities values per Norway (EPC sectoral)
pb_header = {'Category', 'PB_global', 'PB_norway_EPCsect'};

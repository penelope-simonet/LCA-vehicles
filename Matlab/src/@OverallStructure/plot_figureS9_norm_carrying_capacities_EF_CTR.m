function plot_figureS9_norm_carrying_capacities_EF_CTR(obj)

% Same as plot_normalization_EF_planetary_boundaries.m, but Norway's
% planetary boundary allocation uses the Capacity to Reduce (CTR)
% principle instead of Equal Per Capita (EPC).
% Source for PBs: Sala et al. (2020), Table 3.
% Source for CTR allocation: Bjørn et al. (2025), JRC142099, Step 7.3.

% Use : OS.plot_figureS9_norm_carrying_capacities_EF_CTR()

NormFactors = get_normalization_factors_EF_CTR();
PB_norway = NormFactors.PB_CTR;
...
ylabel(ax, 'Share of Norwegian carrying capacities (CTR, log scale)', 'FontSize', 10);
grid(ax, 'on');
box(ax, 'off');
set(ax, 'FontSize', 10);
...
output_pdf = fullfile(output_dir, 'figureS9_norm_carrying_capacities_EF_CTR.pdf');
...
output_xlsx = fullfile(source_dir, 'figureS9_norm_carrying_capacities_EF_CTR.xlsx');
...
% Sheet 2 : carrying capacities values per Norway (CTR)
pb_header = {'Category', 'PB_global', 'PB_norway_CTR'};

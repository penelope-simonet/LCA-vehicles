function plot_figureS13_norm_global_ReCiPe_midpoints(obj)

% Use : OS.plot_figureS13_norm_global_ReCiPe_midpoints()
...
output_pdf = fullfile(output_dir, 'figureS13_norm_global_ReCiPe_midpoints.pdf');
...
for cat = 1:n_cats
    ...
    legend({'Embodied','Operational','Total'},'Location','best','FontSize',10,'Box','off');
    xlabel('Year'); ylabel(units_fleet{cat});
    title(['Fleet : time series : ' cat_name],'FontSize',12,'FontWeight','bold');
    grid on; box off;

    subplot(1,2,2);
    bar(years, [emb_fl', ops_fl'], 'stacked');
    colororder([[0.20 0.45 0.70];[0.85 0.33 0.10]]);
    legend({'Embodied','Operational'},'Location','best','FontSize',10,'Box','off');
    xlabel('Year'); ylabel(units_fleet{cat});
    title(['Fleet : stacked bars : ' cat_name],'FontSize',12,'FontWeight','bold');
    grid on; box off;
...
output_xlsx = fullfile(source_dir, 'figureS13_norm_global_ReCiPe_midpoints.xlsx');

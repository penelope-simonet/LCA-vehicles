function plot_figureS7_norm_carrying_capacities_EF_EV(obj)

% Same as plot_normalization_EF_planetary_boundaries.m, but Norway's
% planetary boundary allocation uses the Economic Value (EV) principle
% instead of Equal Per Capita (EPC).
% Source for PBs: Sala et al. (2020), Table 3.
% Source for EV allocation: Bjørn et al. (2025), JRC142099, Table 2.

% Use : OS.plot_figureS7_norm_carrying_capacities_EF_EV()

NormFactors = get_normalization_factors_EF_ECO();
PB_norway = NormFactors.PB_EV;
...
text(ax, years(end)+0.5, 1, sprintf('Carrying capacities, EV'), ...
    'FontSize', 10, 'Color', 'r', 'VerticalAlignment', 'middle', ...
    'HorizontalAlignment', 'left');

set(ax, 'YScale', 'log');
xlabel(ax, 'Year', 'FontSize', 10);
ylabel(ax, 'Share of Norwegian carrying capacities (EV, log scale)', 'FontSize', 10);
grid(ax, 'on');
box(ax, 'off');
set(ax, 'FontSize', 10);
...
output_pdf = fullfile(output_dir, 'figureS7_norm_carrying_capacities_EF_EV.pdf');
...
output_xlsx = fullfile(source_dir, 'figureS7_norm_carrying_capacities_EF_EV.xlsx');
...
% Sheet 2 : carrying capacities values per Norway (EV)
pb_header = {'Category', 'PB_global', 'PB_norway_EV'};

function NormFactors = get_normalization_factors_EF_ECO()
% Same global planetary boundaries as get_normalization_factors_EF(), but
% allocated to Norway using the "Economic Value" (EV) principle instead of
% Equal Per Capita (EPC) or Capacity to Reduce (CTR).
%
...
%
% Definition (Table 2): "The carrying capacity allocated to an activity is
% proportional to its economic value (generation), for example as measured
% by value added (~contribution to GDP) or prices."
%
% AF_EV,S = GDP_S / Sum_S(GDP_S)
% (GDP_S = TOTAL GDP of country S, not per capita — unlike CTR)
%
% Computed for reference year 2010 (consistent with EPC/CTR allocations
% used elsewhere in this codebase), from World Bank data:
%   - Population, total (SP.POP.TOTL)
%   - GDP per capita, PPP, current international $ (NY.GDP.PCAP.PP.CD)
%   - Total GDP (PPP) = population x GDP per capita PPP
%   - 200 countries with both indicators available in 2010
%   - Norway: total GDP (PPP) = $2.846e+11
%   - Sum_S(GDP_S) over 200 countries = $8.988e+13 (approx.)
% See fiche_recap_allocation_EV.md for the full derivation.

AF_EV_norway = 3.1664e-3;
...
NormFactors.AF_EV_norway = AF_EV_norway;
NormFactors.PB_EV        = NormFactors.PB_global * AF_EV_norway;
